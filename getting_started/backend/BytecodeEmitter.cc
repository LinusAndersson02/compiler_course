...
class Emitter {
public:
  explicit Emitter(const IRProgram &ir) : ir_(ir) {
  ...
  void emitFunction(const IRFunction &fn) {
    ...
    for (size_t i = 0; i < fn.code.size(); ++i) {
      ...
      case IRKind::Call: {
        ...
        if (callee == -2) {
          program_.warnings.push_back("Ambiguous call target '" + in.op + "' in function " +
                                      fn.name);
          BCInstr p;
          p.op = OpCode::PushNil;
          out.code.push_back(p);
        } else if (callee < 0) {
          program_.warnings.push_back("Unknown call target '" + in.op + "' in function " +
                                      fn.name);
          BCInstr p;
          p.op = OpCode::PushNil;
          out.code.push_back(p);
